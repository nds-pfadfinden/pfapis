...
class Entry(UserDict):
    ...
    def comments(self):
        if not hasattr(self, '_comments'):
            comments_json = self.form.api.make_call('https://%s.wufoo.com/api/v3/forms/%s/comments.json?entryId=%s' % (
            self.form.api.account, self.form.Hash, self['EntryId']))
...
class Form(WufooObject):
    ...
    def get_entries(self, page_start=0, page_size=100, sort_field='DateCreated', sort_direction='DESC',
                    filter_string=None):
        url = "%s?system=true&pageStart=%s&pageSize=%s&sort=%s&sortDirection=%s" % (
        self.LinkEntries, page_start, page_size, sort_field, sort_direction)
        if filter_string:
    ...
    def comments(self):
        if not hasattr(self, '_comments'):
            comments_json = self.api.make_call(
                'https://%s.wufoo.com/api/v3/forms/%s/comments.json' % (self.api.account, self.Hash))
...
class Widget(WufooObject):
    def get_embed_code(self):
        if hasattr(self, 'Hash'):
            return '<script type="text/javascript">var host = (("https:" == document.location.protocol) ? "https://" : "http://");document.write(unescape("%%3Cscript src=\'" + host + "%s.wufoo.com/scripts/widget/embed.js?w=%s\' type=\'text/javascript\'%%3E%%3C/script%%3E"));</script>' % (
            self.api.account, self.Hash)
        else:
            return None
